'''Compute IEQ values for opers Stokes data of examples in the Dumas-Neitzke experimental paper.'''
import sys
import json
import itertools
import subprocess
...
def computetheory(tn,thetalist,**kwargs):
    ...
    for theta in thetalist:
        for absh in th['abshlist']:
            yield 'import integralequations as ieq; _,fn = ieq.computeAndSaveXar(\'{theoryname}\',oper=True,savexar=False,saveclusters=True,thetalist=[{theta}],abshlist=[{absh}]); print(fn)'.format(
            theoryname=tn,
            absh=absh,
            theta=theta
            )
...
def inventorytheory(tn,thetalist,**kwargs):
    ...
    for theta in thetalist:
        for absh in th['abshlist']:
            yield namegen.ie_filename(tn,oper=True,absh=absh,theta=theta,clusteronly=True,**kwargs)
